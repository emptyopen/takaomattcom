...
### To-do
'''
incorporate find_good_stocks into this class
'''

with open(os.pardir + '/auth/oauth.txt') as f:
	myAPI = f.read()
quandl.ApiConfig.api_key = myAPI
...
class CapitVita(object):
    ...
    def __init__(self, title = '', num_stocks = 15, style = 'stock',  # style can be stock or option
        home_path = '', mailing_list = [], debug = False):

        self.title = title
        self.num_stocks = num_stocks
        self.style = style
        self.home_path = home_path
        self.file_path = home_path+'data/'
    ...
    def graph_data(self, signal_name, rng = 150, style = 'lowcost', saveLocation = ''):
        ...
        try:
            ...
            highp = self.df['high']
            lowp = self.df['low']
            date = date.apply(lambda x: mdates.date2num(x.to_pydatetime()))

            #ns = 1e-9
            #date = [datetime.datetime.utcfromtimestamp(x.astype(int)*ns) for x in date]
            ...
            plt.suptitle(signal_name,color=spineColor)
            ...
            # plot 0 ---------------------------------------------------------------------------------------
            ax0 = plt.subplot2grid((rowNum,colNum),(0,0),rowspan=1,colspan=4,axisbg='w')
            ...
            # plot 1 ---------------------------------------------------------------------------------------
            ax1 = plt.subplot2grid((rowNum,colNum),(1,0),rowspan=5,colspan=4,axisbg='w',sharex=ax0)
            ...
            d1 = int((min1+max1)/2)
            d2 = int((min2+max2)/2)
            ax1.plot([date[d1],date[d2]],[closep[d1],closep[d2]],linewidth=10,color='#4ee6fd',alpha=0.8,linestyle=':')
            ...
            # plot 2 ---------------------------------------------------------------------------------------
            ax2 = plt.subplot2grid((rowNum,colNum),(6,0),rowspan=1,colspan=4,axisbg='w',sharex=ax0)
            ...
            ax2.fill_between(date, self.df['MACD_norm'], 0, alpha=0.5, facecolor='#4ee6fd')
            ...
            ax2v.fill_between(date, self.df['MACD_der'], 0, alpha=0.5, facecolor='#eaed4d')
            ...
            # plot 5 ---------------------------------------------------------------------------------------
            cLength = 7
            ax5 = plt.subplot2grid((rowNum,colNum),(8,0),rowspan=2,colspan=2,axisbg='w')
            ...
            # plot 6 ---------------------------------------------------------------------------------------
            ax6 = plt.subplot2grid((rowNum,colNum),(8,2),rowspan=2,colspan=1,axisbg='w')
            ax6.grid(False)
            ...
            if style == 'options':
                ...
                candlestickFactor = 1.2


            patterns = self.detectCandlestickPatterns(openp[-7:],closep[-7:],lowp[-7:],highp[-7:],candlestickFactor)

            for i in patterns:
                patternText += i[0] + ' ' + str(7-i[1]) + ' days ago: '+str(i[1])+' points\n'

            patternText = patternText[:-1]

            if len(patternText) == 0:
                patternText = 'no patterns'

            # show patterns
            plt.text(0.5, 0.5, patternText, size=10, color = 'black',
                ha="center", va="center",
            ...
            # plot 7 ---------------------------------------------------------------------------------------
            ax7 = plt.subplot2grid((rowNum,colNum),(8,3),rowspan=2,colspan=1,axisbg='w')
            ax7.grid(False)
    ...
    def get_points(self, signal_name, criteria = {}, rng = '1y', style = 'lowcost'):
    ...
    def grab_data(self, signal_name, rng = 150):
    ...
    '''
    def grabAllData(self, stockList, rng = 365):
        stockList = ['WIKI/'+x for x in stockList]
        return quandl.get(stockList, start_date = '2016-10-01')
    '''
    ...
    def create_stock_files(self):  ### update this one
        if os.path.isfile(self.style+'.txt'):
            with open(home_path+'stockListAll.txt', 'r') as f:
                stockset = list(f.read().split(','))
            stock_file = open(self.file_path+'stocks.txt', 'rw')
            option_file = open(self.file_path+'options.txt', 'rw')
            for stock in stockset:
                self.grab_data(stock)
                last_close = self.df['close'].iloc[-1]
                if last_close < 35 and last_close > 7:
                    stock_file.write(stock)
                    stock_file.write(',')
                if last_close < 35 and last_close > 7:
                    option_file.write(stock)
                    option_file.write(',')
            stock_file.close()
            option_file.close()
    ...
    def find_stocks(self, graph = False):
        ...
        # if there is a pre-existing style within 10 days, use that list of stocks
        # list should contain stocks that are > $10 from max limit

        if self.style == 'stock' and os.path.isfile(self.home_path+'20170718-stock.txt'):
            print('  Using weekly :)')
            with open(home_path+'20170718-robinhood.txt', 'r') as f:
                stockset = list(f.read().split(','))[:-2]
        elif self.style == 'option' and os.path.isfile(self.home_path+'20170718-option.txt'):
            print('  Using weekly :)')
            with open(home_path+'20170718-options.txt', 'r') as f:
                stockset = list(f.read().split(','))[:-2]
        else:
            print('  Using all :(')
            with open(home_path+'stockListAll.txt', 'r') as f:
                stockset = list(f.read().split(','))
        if self.debug:
            stockset = stockset[:30]
        lenStockset = len(stockset)
        print('  Expect script to take approximately {} minutes'.format(round(lenStockset*1.0613 - 14)/60, 2))

        # grab data in batches
        print('Grabbing Quandl data for {} stocks...'.format(lenStockset))
        ...
        if len(self.mailing_list) > 0:
            send_email(self.file_path, self.title+' Top '+str(self.num_stocks)+' Prospects', self.mailing_list)
